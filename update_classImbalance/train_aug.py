...
class CustomDatasetWithAugmentation(Dataset):
    ...
    def __len__(self):
        return len(self.dataset) * 4  # 원본 이미지와 세 가지 증강 이미지
...
class Classifier():
    ...
    def train(self, train_loader, val_loader, test_loader, epochs=10, lr=0.0001):
        cls_num_list = [len(np.where(np.array(train_loader.dataset.dataset.targets) == i)[0]) for i in range(len(train_loader.dataset.dataset.classes))]
