...
def read_image(file: UploadFile):
    contents = file.read()  # 이미지 파일을 Byte로 읽음
    return contents

@measure_time
def save_image(contents, file):
...
@measure_time
def save_to_db(image_id, file, filepath, hash, size):
...
async def upload_image(file: UploadFile = File(...)):
    ...
    contents = await read_image(file)
    filepath, hash, size = save_image(contents, file)
    save_to_db(image_id, file, filepath, hash, size)
    send_to_kafka(image_id)
...
# 모델 설정
model = models.resnet18(pretrained=True)    # models.convnext_base(pretrained=True) resnet18 resnet50
model.eval()
...
@measure_time
def preprocess_center_crop(image):
    return transforms.CenterCrop(224)(image)
...
@measure_time
def search_similar_in_weaviate(embedding, top_n):
    client = weaviate.Client(WEAVIATE_URL)
...
#         query_result = search_similar_in_weaviate(embedding, top_n)
...
# 저장된 벡터로 검색 진행
# 기존 /search_similar 엔드포인트를 수정하여 무작위 벡터를 사용
@app.post("/search_similar/", response_model=List[SimilarImageResponse])
async def search_similar():
    ...
    try:
        ...
        """
        total_count_result = client.query.raw(total_count_query)
        total_count = total_count_result['data']['Aggregate']['ImageEmbedding'][0]['meta']['count']
        random_offset = random.randint(0, total_count - 1)
        
        query_result = client.query.get("ImageEmbedding", ["embedding"]).with_limit(1).with_offset(random_offset).do()
        ...
        for result in search_result['data']['Get']['ImageEmbedding']:
            ...
            cosine_similarity = weaviate_certainty_to_cosine(certainty)
            results.append(SimilarImageResponse(image_id=result['uuid'], score=cosine_similarity))
...
async def get_vector_count():
    ...
    try:
        ...
        count = result['data']['Aggregate']['ImageEmbedding'][0]['meta']['count']
        return {"vector_count": count}
    except Exception as e:
        print(f"Error during fetching vector count: {e}")
        return JSONResponse(status_code=500, content={"message": "Internal Server Error"})
