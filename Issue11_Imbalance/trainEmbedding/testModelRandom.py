    ...
    def __init__(self, model_name, cuda=False):
        ...
        try:
            if model_name.startswith("coca_"):
                self.model, _, self.transform = create_model_and_transforms(model_name, pretrained=None)
            elif hasattr(models, model_name):
                self.model = getattr(models, model_name)(pretrained=False).to(self.device)
            else:
...
def classify_images(model_dict, train_folder, test_folder, cuda=False):
    ...
    for model_display_name, model_name in model_dict.items():
        ...
        if model_name.startswith("coca_"):
            img2vec = CoCaImg2Vec(model_name, pretrained=None, cuda=cuda)
        else:
...
if __name__ == "__main__":                                                                       
    train_folder = '/Users/hahyeonji/Documents/AIV_Intern/ImageImbedding/data-gatter/TRAIN_SET_NR_S'  # TRAIN_SET_A : 전체 데이터  TRAIN_SET_R : RING 기준  TRAIN_SET_NR : DOT(42) 기준
