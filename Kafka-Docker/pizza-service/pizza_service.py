...
producer_config = dict(config_parser['kafka_producer'])
consumer_config = dict(config_parser['kafka_producer'])
consumer_config.update(config_parser['kafka_consumer'])
...
def get_order(order_id):
    order = pizza_warmer.get(order_id)
    if order is None:
